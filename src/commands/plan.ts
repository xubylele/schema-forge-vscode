...
export async function planCommand(): Promise<void> {
  ...
  const command = localCliPath
    ? `node "${localCliPath}" plan --force`
    : 'npx --yes @xubylele/schema-forge plan --force';

  if (localCliPath) {
    logInfo(`Using local CLI: ${localCliPath}`);
  } else {
    logInfo('Using npx to run schema-forge plan');
  }

  logInfo(`Working directory: ${workspaceFolder.uri.fsPath}`);
  logInfo('Starting migration plan preview...');
  logInfo('');
  ...
  childProcess.on('close', (code: number | null) => {
    ...
    if (code === 0) {
      void vscode.window.showInformationMessage('Schema Forge plan preview generated successfully!');
    } else {
      void vscode.window.showErrorMessage('Schema Forge plan preview failed. Check output for details.');
    }
  });
